...
# 为了防止过拟合，在pre-train的过程中构造一些短的样本
...
# 最后是将处理好的数据保存为tfrecord文件。首先将token转为id，增加input_mask用于记录实句长度
# 最后将不到最大长度的部分用0补齐
...
# 一个序列在指定MASK数量之后，有80%被真正MASK，10%还是保留原来token，10%被随机替换成其他token
# 这部分对token进行随机mask。这部分是BERT的创新点之二，随机遮蔽。
# 为了防止双向模型在多层之后“看到自己”。这里对一部分词进行随机遮蔽，并在预训练中进行预测
...
def main(_):
    ...
    # 它以vocab_file为词典，将词转化为该词对应的id，对于某些特殊词，如johanson，
    # 会先将johanson按照最大长度拆分，再看拆分的部分是否在vocab_file里
